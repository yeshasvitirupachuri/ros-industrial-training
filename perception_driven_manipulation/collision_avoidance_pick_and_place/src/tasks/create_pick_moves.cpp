...
std::vector<geometry_msgs::Pose> collision_avoidance_pick_and_place::PickAndPlace::create_pick_moves(geometry_msgs::Pose &box_pose)
{
  ...
  ROS_INFO_STREAM("box_pose position : " << "[" << box_pose.position.x << ", " << box_pose.position.y << ", " << box_pose.position.z << "]");
  ...
  transform_listener_ptr->lookupTransform(cfg.TCP_LINK_NAME, cfg.WRIST_LINK_NAME, ros::Time::now(), tf);
   ...
   */
  transform_from_tcp_to_wrist(tf, wrist_pick_poses);
